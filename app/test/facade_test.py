...
from ..libs.model import User, Todo
...
class facade_test(DatabaseTestCase):
    ...
    def test_can_find_todos_by_owner_if_user_is_not_registed(self):
        user = User(
            username="username1",
            password="password1",
    ...
    def test_find_todo_by_id(self):
        ...
        todo = Todo(content="Hello1", owner=user, required_clock=3)
    ...
    def test_find_todo_by_id_should_return_none_if_todo_not_exists(self):
        ...
        self.session.add(user)
        todo = Todo(content="Hello2", owner=user, required_clock=3)
        self.session.add(todo)
        self.session.commit()
    ...
    def test_push_while_stack_is_empty(self):
        ...
        todo = Todo(content="Hello3", owner=user, required_clock=3)
    ...
    def test_push_the_second_todo_s_order_is_1(self):
        ...
        self.facade.push_todo(user, todo)
        self.assertEquals(0, todo.order)
        todo = Todo(content="World", owner=user, required_clock=3)
        self.facade.push_todo(user, todo)
    ...
    def test_append(self):
        ...
        for i in range(1, 11):
            todo = Todo(content=str(i), owner=user, required_clock=3)
            self.facade.push_todo(user, todo)
        todo = Todo(content=str(0), owner=user, required_clock=3)
        todos = sorted(
            self.facade.append_todo(user, todo),
    ...
    def test_move_todo_to_trash(self):
        ...
        todos = filter(lambda todo: not todo.in_trash, todos)
        self.assertEquals("4", todos[4].content)
        self.facade.move_todo_to_trash(todo=todos[4], user=user)
        todos_after_move_one_to_trash = filter(
            lambda todo: not todo.in_trash,
    ...
    def test_move_todo_while_nothing_to_mvoe(self):
        ...
        self.assertEquals(
            None,
            self.facade.move_todo(user=user, fromOrder=1, toOrder=2)
        )
    ...
    def test_clean_trash(self):
        ...
        for i in range(10):
            ...
            self.facade.push_todo(user, todo)
        todos = self.facade.find_todos_by_owner(user)
        in_trash_todos = [todo for todo in todos if todo.in_trash is True]
    ...
    def test_remove_todo(self):
        ...
        self.facade.remove_todo(user=user, todo=todos[0])
    ...
    def test_find_user_by_credential_normal(self):
        ...
        find_user = self.facade.find_user_by_credential(
            "username1",
            "password1"
        )
        self.assertEquals(user.id, find_user.id)

    def test_increase_one_remaining_clock(self):
        user = self.facade.register(
            "username1",
            "password1",
            "username@domain.name"
        ...
        self.assertEquals(0, todo.consumed_clock)
        self.assertEquals(0, todo.extended_clock)
        self.facade.increase_consumed_clock(todo)
        self.assertEquals(3, todo.required_clock)
        self.assertEquals(1, todo.consumed_clock)
        self.assertEquals(0, todo.extended_clock)

    def test_set_extended_clock(self):
        user = self.facade.register(
            "username1",
            "password1",
            "username@domain.name"
